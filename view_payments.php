...
$id = filter_input(INPUT_GET, 'id');
 $db->join("`client_bike_details` e", "d.bike_id = e.id", "inner");
  $db->join("m_client c", "c.id = e.client_id", "inner");
  $db->join("client_payment s", "s.insurance_id = d.id", "inner");
   // $db->join("payments_to_monarch p", "p.id = s.payment_id", "left");
            $db->where("c.status_enum", 300);
           //   $db->where("c.office_id", 50);
            $db->where("d.cancelled", '0');
             $db->where("s.payment_id", $id);
          //  $db->where("d.print_status", "0");
             $db->groupBy('s.id');
   $select = array('s.id as id','if(d.cover_type="comp",1625,525 ) AS amount', 'c.display_name','d.policy_number','d.commence_date','d.period', 'd.due_date','c.mobile_no','c.account_no',"e.`number_plate`","e.Engine","e.Model","e.Others","e.client_id");
    ...
    <div class="text-center">
        ...
        if ($total_pages > 1) {
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
                ...
                echo '<li' . $li_class . '><a href="view_payments.php' . $http_query . '&page=' . $i . '">' . $i . '</a></li>';
            }
            echo '</ul></div>';
        }
        ?>
